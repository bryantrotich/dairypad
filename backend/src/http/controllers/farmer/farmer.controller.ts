...
import { AdvanceModel, FarmerModel, SalaryModel, UserModel } from "src/database/models";
import { CreateSalaryValidation } from "src/http/validations";
import { Permissions } from "src/support/gates";
import { CreateFarmerValidation } from "src/http/validations/farmer/create.validation";
...
export class FarmerController {
    ...
    async index(
        ...
        try {
            ...
            let [ farmers, count ] = await this.farmerModel.findAndCount({ society: user.society },options);

            // Get pages
            let pages = Math.ceil(count / limit);
            
            // Send the fetched societies as a JSON response with HTTP status 200
            res.status(HttpStatus.OK).json({ farmers, count, pages });
        } catch (error) {
    ...
    @Permissions('CREATE_FARMERS')
    @Post('')
    /**
     * Store a newly created society in storage.
     * 
     * @param {CreateFarmerValidation} body - The request body containing the society data.
     * @param {Request} req - The HTTP request object.
     * @param {Response} res - The HTTP response object.
     * 
     ...
     */
    async store(
        @Body() body: CreateFarmerValidation,
        @Req()  req:  Request,  
        @Res()  res:  Response
    ): Promise<void> {
        try {
            ...
            await this.farmerModel.save(body);
    ...
    async fetch(
        ...
        try {
            // Fetch all expense types
            let farmers = await this.farmerModel.find({});

            // Send the fetched expense types as a JSON response with HTTP status 200
            res.status(HttpStatus.OK).json({ farmers });
        } catch (error) {
            // Log the error and throw an HTTP exception with the error message and status
            console.log(error);
            throw new HttpException(error.message, error.status);
