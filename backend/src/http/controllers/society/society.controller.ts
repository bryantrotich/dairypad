...
import { SocietyModel } from 'src/database/models';
...
export class SocietyController {
    ...
    async index(
        ...
        @Query('per_page', new DefaultValuePipe(Number(5))) per_page: number,
        ...
        try {
            ...
            let societies = await this.societyModel.findByCursor({},{ first: per_page, after: String(per_page), orderBy: { [order_by]: order} });

            // Send the fetched societies as a JSON response with HTTP status 200
            res.status(HttpStatus.OK).json({ societies });
        } catch (error) {
